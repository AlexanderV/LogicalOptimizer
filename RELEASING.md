...
`release-evidence-<version>` **і** прикріплюється до GitHub release разом із `.nupkg`/`.snupkg` та
`SHA256SUMS.txt`. Release створюється самим workflow, якщо для тега його ще немає — нотатки беруться
з `claim-changes.md`, тобто із секції CHANGELOG цієї версії. Створювати release руками не треба.

> **Чому це так.** Раніше крок прикріплював файли лише до **вже створеного** release, а інакше
> друкував примітку й завершувався успішно. Саме так вийшов v3.2.2: дев'ять пакетів опубліковано,
> job зелений, release не існує — а `verifying-provenance.md` посилається на файли, яких ніде немає.
> Тепер release створюється, склад активів перевіряється після завантаження, а наявність секції
> CHANGELOG для версії перевіряється **на початку job**, до `pack` і `push`: крок нотаток працює
> після публікації в nuget.org, і падіння там лишило б пакети опублікованими назавжди.
