# LogicalOptimizer.Core (deprecated forwarding package)

**Since v4.0 the whole toolkit ships as the single package
[LogicalOptimizer](https://www.nuget.org/packages/LogicalOptimizer/).**

LogicalOptimizer.Core contains no code anymore: it only depends on `LogicalOptimizer`,
...
```bash
dotnet remove package LogicalOptimizer.Core
