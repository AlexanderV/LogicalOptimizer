...
- [ ] **Реліз v4.0.0 (рішення мейнтейнера, незворотне):** замінити `## [4.0.0] - Unreleased` на дату → `git tag v4.0.0 && git push origin v4.0.0` → після publish поставити deprecated-позначки на 7 forwarding-ID на nuget.org (крок з ADR). Нічний Exhaustive workflow додатково підтвердить sweep-и на GitHub-раннері (03:20 UTC).
