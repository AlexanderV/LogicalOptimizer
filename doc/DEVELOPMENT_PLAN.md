...
- [ ] **Зовнішня верифікація (єдиний залишок; runbook):**
  1. Закомітити й запушити зміни в `main` → зелені `ci.yml` (обидві ОС: gate, pack `--no-build` ×9, contract) і `docs.yml`.
  2. Actions → Release → **Run workflow** (dry run) → зелений прогін = перший реальний release-ланцюг із pre-publish Native AOT на Linux runner, без публікації; перевірити артефакт `dry-run-evidence-4.0.0` (зокрема `source` і `consolidatedPackageSha256` в AOT-звіті).
  3. За бажанням: Actions → Exhaustive sweeps → Run workflow (нічний прогін і так стартує о 03:20 UTC).
  4. Коли dry run зелений — `git tag v4.0.0 && git push origin v4.0.0` (незворотна публікація; перед цим замінити `## [4.0.0] - Unreleased` на дату релізу); після publish вручну поставити deprecated-позначку на 7 forwarding-ID на nuget.org (крок з ADR).
